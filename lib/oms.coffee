...
class @['OverlappingMarkerSpiderfier']
  ...
  x['VERSION'] = '0.3.4' for x in [@, p]  # better on @, but defined on p too for backward-compat
  ...
  fromLatLngToPixel: (position) ->
    ...
    point = proj.fromLatLngToPoint(position)
  ...
  p.llToPt = (ll) -> @fromLatLngToPixel(ll)
  p.ptToLl = (pt) -> @fromPixelToLatLng(pt)
